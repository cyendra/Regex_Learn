...
namespace regex_internal
{
	...
	***********************************************************************/
	...
	// 创建一个新的从start状态到end状态的边
	// 是状态start的出边，状态end的入边
	...
	***********************************************************************/
	...
	//递归保证转换先后顺序
	void CollectEpsilon(State* targetState, State* sourceState, bool(*epsilonChecker)(Transition*), std::list<State*>& epsilonStates, std::list<Transition*>& transitions)
	{
		if (std::find(epsilonStates.begin(), epsilonStates.end(), sourceState) == epsilonStates.end())
		{
			epsilonStates.push_back(sourceState);
			for (int i = 0; i<sourceState->transitions.size(); i++)
			{
				Transition* transition = sourceState->transitions[i];
				if (epsilonChecker(transition))
				{
					if (std::find(epsilonStates.begin(), epsilonStates.end(), transition->target) == epsilonStates.end())
					{
	...
	Automaton::Ref EpsilonNfaToNfa(Automaton::Ref source, bool(*epsilonChecker)(Transition*), std::map<State*, State*>& nfaStateMap)
	{
		Automaton::Ref target = new Automaton;
		std::map<State*, State*> stateMap;	//source->target
		std::list<State*> epsilonStates;				//每次迭代当前状态的epsilon闭包
		std::list<Transition*> transitions;			//每次迭代当前状态的epsilon闭包的转换集合
